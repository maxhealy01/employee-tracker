		...
		.then(({ choice }) => {
			switch (choice) {
				...
				case "Update Employee Manager":
					this.updateManagerQueries();
					break;
				case "See Expenditures":
					this.seeExpenditures();
					break;
			}
		});
};
			...
			},
			{
				...
				validate: (nameInput) => {
					if (nameInput) {
						...
						console.log("Please enter the name of the role's department.");
						return false;
					}
				},
			},
...
Prompts.prototype.updateRoleQueries = function () {
	inquirer
		...
		.then(({ firstName, lastName, role }) => {
			updateRole(firstName, lastName, role);
			console.log("The employee's role has been updated!");
			this.initializeProgram();
		});
};

Prompts.prototype.updateManagerQueries = function () {
	inquirer
		.prompt([
			{
				type: "text",
			...
			},
			{
				...
				validate: (nameInput) => {
					if (nameInput) {
						...
						console.log("Please enter the employee's new manager.");
						return false;
					}
				},
			},
		...
		.then(({ firstName, lastName, manager }) => {
			updateManager(firstName, lastName, manager);
			console.log("The employee's manager has been updated!");
			this.initializeProgram();
		});
};

Prompts.prototype.seeExpenditures = function () {
	inquirer
		.prompt([
			{
				type: "text",
				name: "department",
				message:
					"Enter the department whose yearly expenditure you'd like to see.",
			},
		])
		.then(({ department }) => {
			console.table(seeExpendituresNow(department));
			this.initializeProgram();
		});
};
